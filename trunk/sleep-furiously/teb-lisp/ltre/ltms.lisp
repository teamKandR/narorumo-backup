...
(defun enable-assumption (node label)
  ...
  ;;(format t "** I'm enable-assumption! node: ~a, label: ~a~%"
  ;; node label)
  ...
  (cond ((not (tms-node-assumption? node))
	...
	(t
	 (ltms-error "Can't set an already set node" node))))
...
(defun top-set-truth (node value reason &aux *clauses-to-check*)
  ...
  ;;  (format t "top-set-truth!! node: ~a, value: ~a, reason: ~a, ctc: ~a~%"
  ;; node value reason *clauses-to-check*)
